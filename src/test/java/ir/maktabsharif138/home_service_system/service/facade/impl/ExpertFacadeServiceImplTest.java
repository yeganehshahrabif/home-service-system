...
class ExpertFacadeServiceImplTest {
    ...
    void register_shouldReturnResponse_whenImageExists() {
        ...
        MultipartFile image = new MockMultipartFile("img", "a.jpg", "image/jpg", new byte[]{1});

        Expert expert = new Expert();
        Expert saved = new Expert();
        ExpertResponse response = mock(ExpertResponse.class);

        when(expertMapper.toExpert(request)).thenReturn(expert);
        when(fileStorageService.saveProfileImage(image)).thenReturn("path.jpg");
        when(expertCoreService.register(expert)).thenReturn(saved);
        when(expertMapper.toExpertResponse(saved)).thenReturn(response);

        ExpertResponse result = facade.register(request, image);

        assertEquals(response, result);

        verify(fileStorageService).saveProfileImage(image);
        verify(expertCoreService).register(expert);
    }
    ...
    void register_shouldWork_whenNoImage() {
        ...
        ExpertResponse result = facade.register(request, null);

        assertEquals(response, result);

        verify(fileStorageService, never()).saveProfileImage(any());
    }
    ...
    @Test
    void updateProfile_shouldUpdateWithImage() {

        ExpertUpdateRequest request = new ExpertUpdateRequest();
        MultipartFile image = new MockMultipartFile("img", "a.jpg", "image/jpg", new byte[]{1});
        ...
        when(expertCoreService.findById(expertId)).thenReturn(expert);
        when(fileStorageService.saveProfileImage(image)).thenReturn("new.jpg");
        when(expertCoreService.update(expert)).thenReturn(saved);
        when(expertMapper.toExpertResponse(saved)).thenReturn(response);

        ExpertResponse result = facade.updateProfile(expertId, request, image);
    ...
    @Test
    void updateProfile_shouldWorkWithoutImage() {
        ...
        ExpertResponse result = facade.updateProfile(expertId, request, null);
